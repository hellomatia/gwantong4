...
function searchTouristSpots() {
    ...
    var url = `${ root }/attraction?action=map&areaCode=`+areaCode+`&contentTypeId=`+contentTypeId;

    fetch(url)
        .then(response => response.json())
        .then(data => {
            if(data.data.response.header.resultCode === "0000") {
                displayTouristSpots(data.data.response.body.items.item);
                displaySearchResults(data.data.response.body.items.item);
            } else {
                console.error('Error:', data.data.response.header.resultMsg);
            }
        })
...
function displaySearchResults(spots) {
    ...
    spots.forEach((spot, index) => {
        ...
        var imageSrc = spot.first_image ? spot.first_image : '${ root }/img/main2.png'; // 이미지가 없으면 기본 이미지로 설정
...
function displayTouristSpots(spots) {
    ...
    spots.forEach(spot => {
        ...
        kakao.maps.event.addListener(marker, 'click', function() {
            ...
            var imageToShow = spot.first_image ? spot.first_image : '${ root }/img/main2.png';
